...
public class StudentController {
    ...
    public String home(@RequestParam(required = false) String keyword,
        ...
        List<Student> students =
                service.searchStudentsByUsername(
                        principal.getName(),
                        keyword
                );
    ...
    public String saveStudent(@ModelAttribute Student student,
        ...
        if (student.getId() != null) {
            ...
            Student oldStudent =
                    service.getStudentById(student.getId());
    ...
    public void downloadReport(@PathVariable Long id,
        ...
        response.setHeader(
                "Content-Disposition",
                "attachment; filename=career-report-" + student.getId() + ".pdf"
        );
        ...
        document.add(
                new Paragraph(
                        service.getCareerSuggestion(student),
                        normalFont
                )
        );
        document.add(new Paragraph(" "));
        ...
        document.add(
                new Paragraph(
                        service.getCareerRoadmap(student),
                        normalFont
                )
        );
    ...
    private void addStudentDashboardData(Model model,
                                         String username) {

        List<Student> students =
                service.getStudentsByUsername(username);

        long tenthCount =
                students.stream()
                        .filter(s -> "10th".equalsIgnoreCase(s.getQualification()))
                        .count();

        long twelfthCount =
                students.stream()
                        .filter(s -> "12th".equalsIgnoreCase(s.getQualification()))
                        .count();

        long diplomaCount =
                students.stream()
                        .filter(s -> "Diploma".equalsIgnoreCase(s.getQualification()))
                        .count();

        long degreeCount =
                students.stream()
                        .filter(s -> "Degree".equalsIgnoreCase(s.getQualification()))
                        .count();
